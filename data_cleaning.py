...
def upload_to_hub(dataset_dict, dataset_name):
    ...
    print(
        f"Dataset uploaded successfully to: https://huggingface.co/datasets/{dataset_name}"
    )
...
def filter_responses(example):
    ...
    if (
        example["chosen"][1].get("role") == "assistant"
        and example["chosen"][1].get("content", "").strip() == ""
    ):
        return False
...
if __name__ == "__main__":
    ...
    upload_to_hub(
        filtered_dataset, "pbevan11/multilingual-constitutional-preference-pairs"
    )
